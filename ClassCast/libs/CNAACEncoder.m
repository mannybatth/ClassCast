...
- (instancetype) initWithBitrate:(NSUInteger)bitrate sampleRate:(NSUInteger)sampleRate channels:(NSUInteger)channels {
    if (self = [super initWithBitrate:bitrate sampleRate:sampleRate channels:channels]) {
        self.encoderQueue = dispatch_queue_create("CN Encoder Queue", DISPATCH_QUEUE_SERIAL);
...
{
    CNAACEncoder *encoder = (__bridge CNAACEncoder *)(inUserData);
...
- (void) encodeSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    dispatch_async(self.encoderQueue, ^{
        ...
        if (self.delegate) {
            CNFrame *frame = [[CNFrame alloc] initWithData:data pts:pts];
